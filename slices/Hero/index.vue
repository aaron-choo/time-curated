<template>
  <Bounded as="section" :collapsible="false" class="pb-0 md:pb-0">
...
export default {
  ...
  methods: {
    htmlSerializer(type, element, content, children) {
      switch (type) {
        case "heading1":
          return /* html */ `<h1 class="font-semibold tracking-tighter text-5xl md:text-[5rem] mb-6 last:mb-0">${children.join(
            ""
          )}</h1>`;

        case "paragraph":
          return /* html */ `<p class="mb-6 last:mb-0">${children.join(
            ""
          )}</p>`;
